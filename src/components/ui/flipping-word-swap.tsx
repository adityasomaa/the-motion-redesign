...
export interface FlippingWordSwapProps {
  ...
  /** When set, the words swap on their own every `interval` ms (paused while hovered). */
  interval?: number;
  /** Visual only: removed from the tab order and the accessibility tree (pair it with sr-only text). */
  decorative?: boolean;
}
...
export function FlippingWordSwap({
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      if (!hoveredRef.current && document.visibilityState === "visible") updateSwap(!swappedRef.current);
    }, interval);
  ...
  return (
    <button
      ...
      aria-label={decorative ? undefined : isSwapped ? word2 : word1}
      aria-pressed={decorative ? undefined : isSwapped}
      aria-hidden={decorative || undefined}
      tabIndex={decorative ? -1 : undefined}
      style={style}
      onMouseEnter={() => {
        hoveredRef.current = true;
        updateSwap(!interval ? true : !swappedRef.current);
      }}
      onMouseLeave={() => {
        hoveredRef.current = false;
        if (!interval) updateSwap(false);
      }}
      onPointerUp={(event) => {
