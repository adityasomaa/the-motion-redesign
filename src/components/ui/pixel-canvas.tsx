...
export function PixelCanvas({
    ...
    useEffect(() => {
        ...
        const draw = (timestamp: number) => {
            ...
            ctx.globalAlpha = 1;
            if (visible) animationRef.current = requestAnimationFrame(draw);
        };
        ...
        // Initialize. The loop only runs while the canvas is on screen.
        initPixels();
        ...
        if (!noFocus) {
            (container.parentElement ?? container).addEventListener("mousemove", onMouseMove);
            (container.parentElement ?? container).addEventListener("mouseleave", onMouseLeave);
        ...
        return () => {
            ...
            (container.parentElement ?? container).removeEventListener("mousemove", onMouseMove);
            (container.parentElement ?? container).removeEventListener("mouseleave", onMouseLeave);
